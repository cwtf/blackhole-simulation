...
export const SimulatorApp = ({
  ...
  const massPreset = realObject
    ...
    : findPreset(massPresetId);

  // Whether the live simulation still *is* the object named at the top of the
  // panel. Mass is not checked because it is not user-editable while an object
  // is selected; spin is, via the control panel slider.
  const realObjectLocked = realObject
  ...
  //
  // Keyed on the preset's *fields*, not the preset object: since §6.4 a real
  // object's preset is synthesized fresh on every render, so depending on the
  // object identity would re-fire this effect every render, and the setParams
  ...
  // renderer is mass-invariant (§1.8), so the object's mass reaches the UI
  // through `massPreset` and never touches `params.mass`.
  //
  // Inclination is applied on selection but is deliberately *not* part of the
  // lock check — see `matchesLockedParameters`. Orbiting the camera is the
  // viewer's business; changing the spin is a claim about the black hole.
  const applyRealObject = useCallback(
    (id: string) => {
      const o = findRealBlackHole(id);
      if (!o) return;
      setParams((prev) => ({ ...prev, spin: o.spin.value }));
      setPolarAngle((o.inclinationDeg.value * Math.PI) / 180);
    },
    [setPolarAngle],
  );

  const handleSelectRealObject = useCallback(
    (id: string) => {
      setRealObjectId(id);
      applyRealObject(id);
    },
  ...
  const handleRestoreRealObject = useCallback(() => {
    if (realObjectId) applyRealObject(realObjectId);
  }, [applyRealObject, realObjectId]);

  const handleMassPresetChange = useCallback((id: string) => {
    // Choosing a generic preset leaves the named object behind entirely; the
    // panel should stop claiming to show Cygnus X-1 the moment you pick
    // "Stellar (10 M☉)".
    setRealObjectId(null);
    setMassPresetId(id);
  }, []);

  // Apply the route's object once on mount. Split from the state initialiser
  // because it has to write camera and params, which do not exist yet at
  // initialiser time.
  const appliedInitialObject = useRef(false);
  useEffect(() => {
    if (appliedInitialObject.current || !initialObjectId) return;
    appliedInitialObject.current = true;
    applyRealObject(initialObjectId);
  }, [applyRealObject, initialObjectId]);
    ...
    <div className="relative w-full h-screen bg-black overflow-hidden select-none font-sans text-white">
      ...
      <ErrorBoundary>
        ...
        <TestObjectPanel
          ...
          onMassPresetChange={handleMassPresetChange}
          realObject={realObject}
          realObjectLocked={realObjectLocked}
